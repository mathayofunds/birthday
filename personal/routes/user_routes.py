...
@app.route('/test')
def testmail():
    msg=Message("Testing Mail","ayokunleopadayo@gmail.com",recipients=['receiver@mail.com'])
    if msg:
...
def paycash():
    if session.get('user')!=None:
        if request.method =='GET':
            ...
            t = models.Transaction(trx_guestid=guestid,trx_amt=amount,trx_status='Pending',trx_ref=ref)
            db.session.add(t)
            db.session.commit()
...
def confirmpay():
    #TO DO: Protect this page to only logged in users
    if session.get('user') !=None and session.get('trxref') !=None:
        ref=session.get
        deets=db.session.query(models.Transaction)
        ref =session.get()

    #To DO: Remember that transref is in session?
    #fetch the details of the transaction  fromTransaction table using the trnref in session
    return render_template('user/confirmpay.html') """
